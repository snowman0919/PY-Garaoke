...
class TestKaraokeScorer(unittest.TestCase):
    ...
    def test_extract_pitch(self, mock_pyin, mock_load):
        duration = 5
        analysis_sr = 8000
        dummy_audio, _ = self.create_mock_audio_data(duration)
    ...
    def test_calculate_pitch_accuracy(self):
        ...
        score = self.scorer._calculate_pitch_accuracy(user_f0_perfect, ref_f0_perfect, tolerance_cents=10)
    ...
    def test_calculate_vibrato_quality(self):
        base_f0 = 261.63
        t = np.linspace(0, 1, int(self.sr * 1))
        user_f0_vibrato = base_f0 + 5 * np.sin(2 * np.pi * 5 * t)
        user_f0_vibrato = user_f0_vibrato[::self.hop_length]
    ...
    def test_score_performance(self, mock_vibrato, mock_rhythm, mock_pitch, mock_extract_pitch):
        mock_extract_pitch.side_effect = [
            (np.array([440.0, 440.0]), np.array([0.1, 0.2]), self.sr),
            (np.array([440.0, 440.0]), np.array([0.1, 0.2]), self.sr)
        ]
    ...
    def test_get_realtime_pitch_feedback(self):
        ...
        self.assertEqual(self.scorer.get_realtime_pitch_feedback(440, 430), "Good")
        self.assertEqual(self.scorer.get_realtime_pitch_feedback(440, 400), "Bad")
        self.assertEqual(self.scorer.get_realtime_pitch_feedback(440, 300), "Bad")
